...
def main(dataset: Dataset, question: str) -> Tuple[str, List[str], float, int, int]:
    ...
    answer, files_names, score, start, end = doc_assistant.answer_pipeline(
        master_dataset=dataset, question=question
    )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Answer questions based on a dataset using DocumentAssistant."
    )
    parser.add_argument(
        "--question",
        type=str,
        default=None,
        help="The question you want answered. Leave empty to prompt.",
    )
    ...
    answer, files, score, start, end = main(
        dataset=master_dataset, question=args.question
    )
    output_dict = {
