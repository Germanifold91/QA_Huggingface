...
def prepare_train_data(
    documents: List[Dict[str, Union[str, List[str], List[Dict[str, str]]]]],
    output_path: str,
) -> Dict[str, List[Dict[str, List[Dict[str, List[Dict[str, str]]]]]]]:
    """
            ...
            for qa in questions_answers:
                if qa["context_index"] == i:
                    ...
                    if answer_start != -1:
                        qas.append(
                            {
                                "id": str(question_id).zfill(
                                    5
                                ),  # Use the question ID counter
                                "is_impossible": False,
...
if __name__ == "__main__":
    ...
    prepare_train_data(
        documents=documents, output_path=data_training_params["training_json"]
    )

    # Prepare training dataset
    load_and_format_data(
        data_file=data_training_params["context_qa"],
